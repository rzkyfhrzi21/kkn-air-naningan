...
Berikut adalah aturan prioritas pembacaan file yang wajib dipatuhi oleh AI. **PERINGATAN MANAJEMEN CONTEXT (MENCEGAH CONTENT-BLOCKED):** AI DILARANG membom/membaca belasan file secara beruntun (*eager mass-reading*) di awal sesi. Gunakan metode *Lazy Loading* (membaca file secara bertahap hanya saat relevan dengan tugas/fitur yang sedang dikerjakan).
...
### Ringkasan Fungsi Folder Skill (Referensi Pembacaan Eksplisit / On-Demand):
...
- **📁 TOOL_SKILLS**: Panduan integrasi dan penggunaan alat (tools/MCP) pendukung (dibaca hanya file spesifik saat tool digunakan).
  - `CODEBASE_MEMORY.md`: Digunakan saat membutuhkan pemetaan struktur _repository_.
...
AI membaca panduan dari file dan subfolder di dalam `docs/` secara **bertahap sesuai kebutuhan modul/fitur yang sedang dikerjakan** (*lazy loading*, bukan dibaca sekaligus secara serentak di awal sesi):
...
**DIIZINKAN MEMBACA** (dengan batasan line limit untuk menghindari content-blocked):

- Folder `docs/stitch/` — Mockup HTML Stitch sebagai referensi layout dan struktur UI. **WAJIB gunakan line limit/offset saat membaca**, jangan baca file utuh sekaligus karena berisi ratusan URL gambar eksternal yang bisa memicu `content-blocked`. Prioritaskan ekstrak struktur HTML dan class Tailwind, bukan konten lengkap.
...
Jika pengguna menyatakan bahwa ini adalah proyek atau _workspace_ baru (atau meminta penyiapan *tools*), AI dapat mengingatkan pengguna untuk menjalankan langkah-langkah inisialisasi _tools_ berikut (atau menjalankannya via terminal dengan argumen yang valid):
...
3. **Codebase Memory MCP:** Lakukan pemetaan struktur proyek. Jika server MCP tersambung dan pengguna meminta pemindaian, jalankan aksi pemindaian dengan parameter yang valid.
...
## P.6 Keamanan & Mitigasi Safety Filter
...
### Mitigasi Safety Filter AI (Mencegah Content-Blocked):
1. **Area Risiko Tinggi Dilarang Dibaca Tanpa Izin Eksplisit:** `/secure/`, `docs/DATA KLIEN/`, `admin_credentials.json`, file `.env`.
2. **File HTML Mockup (`docs/stitch/`):** WAJIB gunakan line limit/offset saat membaca. Jangan baca file HTML mockup secara utuh sekaligus karena berisi ratusan URL gambar eksternal. Baca secara bertahap dengan rentang baris kecil (max 200 baris per read).
3. **Pencarian Simbol:** Cari nama fungsi/struktur tanpa membuka file yang memuat nilai rahasia.
4. **Rentang Baris Terbatas:** Gunakan `StartLine` & `EndLine` sekecil yang diperlukan saat membaca file.
5. **Proteksi Kredensial di Output:** Jangan pernah menampilkan ulang kredensial, token, nomor pribadi, atau isi file rahasia (gunakan `[REDACTED]` jika perlu menjelaskan temuan).
6. **Cegah Pembacaan Massal:** Hindari membom pembacaan belasan file sekaligus (*eager mass-reading*) atau *output tool* yang terlalu luas dalam 1 turn.
7. **Penanganan Blokir:** Jika suatu request terblokir, jangan mengulang request yang persis sama; persemplit query/rentang baris.
...
- Jangan melakukan *eager mass-reading* (membaca belasan file docs/skill sekaligus di awal sesi) yang memicu *context overflow* atau *content-blocked*.
- File HTML mockup di `docs/stitch/` DIIZINKAN dibaca untuk referensi layout, TAPI WAJIB gunakan line limit/offset (max 200 baris per read) untuk menghindari content-blocked dari URL gambar eksternal.
